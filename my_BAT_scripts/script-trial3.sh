...
#read folder
folder=GRB210919B # GRB folder name which contains the tarred dataset

#echo "Whats the trigegr time?"
#read trigger_time
trigger_time=653758791.000 # Trigger time in MET 
...
batbinevt tstart=$tstart_lcurve tstop=$tstop_lcurve infile=$inp outfile=grb-lc.fits detmask=$detmask outtype=LC timedel=0.01 timebinalg=u energybins=15-250 weighted=NO outunits=COUNTS clobber=YES
...
batbinevt tstart=$tstart_fgnd tstop=$tstop_fgnd infile=$inp outfile=grb.dpi  detmask=$detmask outtype=DPI timedel=0 timebinalg=u energybins=50-250 weighted=NO outunits=COUNTS clobber=YES

batbinevt tstart=$tstart_bkg tstop=$tstop_bkg infile=$inp outfile=grb-bkg.dpi  detmask=$detmask outtype=DPI timedel=0 timebinalg=u energybins=50-250 weighted=NO outunits=COUNTS clobber=YES
