...
# Commentging out all these arguments 6th July 2022
...
if [ "$#" -ne 2 ]; then # edited to 2 on Aug 29th, 2022
    mintbin=$3
else
    mintbin=0.256
fi
...
if [ -f "filter_evdata.fits" ]; then
    ...
    python $batml_path'do_manage2.py' --GWname $gwname --rhel7 --do_bkg --do_rates --do_llh --queue jak51_b_g_vc_default --q hprc --N_infov_jobs 160 --N_outfov_jobs 40 > manager.out 2>&1 &
